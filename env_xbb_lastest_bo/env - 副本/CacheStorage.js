// CacheStorage对象
CacheStorage = function CacheStorage(){let arg=arguments[0];
if (arg!='bobo' && !(this instanceof CacheStorage) ){return bodavm.toolsFunc.throwError("TypeError", "Illegal constructor")};bodavm.toolsFunc.symbolProperty(this);
if (arg =='bobo'){console.log_copy('CacheStorage 实例化对象 --->',JSON.stringify_bo(arguments,function(k,v){if (v==window){return 'window'}else{return v}}))};}

bodavm.toolsFunc.safeProto(CacheStorage, "CacheStorage");
bodavm.toolsFunc.defineProperty(CacheStorage.prototype, "delete", {configurable:true, enumerable:true, writable:true, value:function bodelete (){return bodavm.toolsFunc.dispatch(this, CacheStorage.prototype, "CacheStorage", "bodelete", arguments)}},);
