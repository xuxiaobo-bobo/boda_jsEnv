...
class Bayes extends Fingerprint {
	generate (inParams, onDone) {
		const generateThread = this.spawn((inParams, onDone, progress) => {
			...
			let trainParams = {
				...
				model: path.join(inParams.tempCatalog, 'dict.bayes'),
				__dirname: inParams.__dirname,
				eventEmitter: GenerationEventEmitter
			};

			bayesTrainer.train(trainParams, (err, status) => {
				if (err)
					throw new Error('Bayes fingerprint generate thread: Error on training! ' + err);
