	...
	acornWalkFull(ast, (node, state, type) => {
		...
		if (nodeType === 'CatchClause') {
			...
			if (param) {
				if (param.type === 'ObjectPattern') {
					insertions.push({
						...
						pos: node.start,
						order: TO_RIGHT,
						code:"" // 修改 code: `catch($tmpname){try{throw ${INTERNAL_STATE_NAME}.handleException($tmpname);}`
					});
					insertions.push({
						__proto__: null,
						pos: node.body.end,
					...
					if (cBody.body.length > 0) {
						insertions.push({
							...
							order: TO_LEFT,
							code:"" // 修改 code: `${name}=${INTERNAL_STATE_NAME}.handleException(${name});`
						});
					}
				}
			}
			...
			insertions.push({
				...
				order: TO_LEFT,
				code:"" // 修改 code: INTERNAL_STATE_NAME + '.wrapWith('
			});
			insertions.push({
				__proto__: null,
				pos: node.object.end,
			...
			insertions.push({
				...
				pos: node.start,
				order: TO_RIGHT,
				code:"" // 修改 code: INTERNAL_STATE_NAME + '.'
			});
		}
	});
