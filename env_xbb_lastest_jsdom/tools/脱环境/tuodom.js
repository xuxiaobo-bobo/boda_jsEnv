getProtoEnvCode = function getProtoEnvCode(proto, instanceObj) {
    ...
    // };
        code += `if (!(this instanceof ${protoName})){return bodavm.toolsFunc.throwError("${e.name}", "${e.message}")};`;
    }
...
getDescriptor = function getDescriptor(obj, prop, objName, protoName, instanceObj) {
    ...
    if (descriptor.hasOwnProperty("value")) {
        ...
        if (prop=='delete') {prop ='bodelete'}
        if (prop =='continue'){prop ='bocontinue'}
    ...
    if (descriptor.hasOwnProperty("get")) {
        if (prop ='default'){prop='bodefault'}
